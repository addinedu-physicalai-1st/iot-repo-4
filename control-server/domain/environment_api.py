...
def control_sensor():
    """웹 GUI의 수동 제어 명령을 호출받아 통합 제어 함수를 통해 실행"""
    from core.sensor_controller import execute_manual_control
    import time
    
    data = request.get_json()
    node_id = data.get('node_id', '').upper()
    device = data.get('device', '').lower() # 'led', 'val', 'fan', 'heater', 'pump'
    state = data.get('state')    # 'ON', 'OFF' 또는 숫자
    ...
    if isinstance(state, str):
        if state.upper() == 'ON': value = 1
        elif state.upper() == 'OFF': value = 0
        elif state.isdigit(): value = int(state)
    elif isinstance(state, (int, float)):
        value = int(state)

    # 1. 수동 모드 전송 (장치가 'mode' 자체가 아닐 경우 선행)
    client_ip = request.remote_addr
    if device != 'mode':
        # MODE(0)를 MANUAL(0)로 설정
        execute_manual_control(node_id, 'mode', 0, trigger=2, source=client_ip)
        time.sleep(0.1)
    
    # 2. 실제 장치 제어 호출 (중앙 함수에서 DB로그, 캐시, 패킷 전송 모두 처리)
    success, msg = execute_manual_control(node_id, device, value, trigger=2, source=client_ip)
    
    if success:
        return jsonify({"ok": True, "message": f"{node_id} {device.upper()} -> {state} 통합 제어 완료"})
    else:
        return jsonify({"ok": False, "error": f"제어 실패: {msg}"}), 503
            ...
            cursor.execute("""
                SELECT * FROM (
                    (SELECT 'CONTROL' as type, logged_at as time, 
                            CONCAT(nc.node_id, ' ', 
                                   CASE na.actuator_type_id 
                                       WHEN 1 THEN '펌프' 
                                       WHEN 2 THEN '팬' 
                                       WHEN 3 THEN '히터' 
                                       WHEN 4 THEN 'LED' 
                                       ELSE '장치' 
                                   END, ' ', state_value) as msg
